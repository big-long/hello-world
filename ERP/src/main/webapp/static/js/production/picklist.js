function refush() {
	...
	$.ajax({
		url : "http://localhost:8080/ERP/picklist/getPicklistByKeyWord",
		type : "get",
		data : {
			keyWord : keyWord,
			pageNum : pageNum,
...
// 修改用户模态框准备数据
function update(obj) {
	var plId = $(obj).parents("tr").find("#plId").text();
	var goodsName = $(obj).parents("tr").find("#goodsName").text();
	var pickNumber = $(obj).parents("tr").find("#pickNumber").text();
	var pickingDate = $(obj).parents("tr").find("#pickingDate").text();
	$.ajax({
		url : "http://localhost:8080/ERP/picklist/getPicklistInfo",
		type : "get",
		data : {
			plId : plId
		},
		success : function(result) {
			if (result.status == 200) {
				var picklist = result.object;
				$("h4").text("修改领料单");
				$("[name=plId]").val(plId);
				$("[name=PicklistName]").val(picklist.purpose);
				$("[name=goodsName]").val(goodsName);
				$("[name=plMan]").val(picklist.plMan);
				$("[name=plPosition]").val(picklist.plPosition);
				$("[name=pickNumber]").val(pickNumber);
				$("[name=operator]").val(picklist.operator);
				$("[name=pickingDate]").val(pickingDate);
				$("[name=remark]").val(picklist.remark);
			} else {
				alert(result.message);
			}
...
function loadPage(result) {
	...
	for (var i = 0; i < data.length; i++) {
		...
		html += "<td id='plId' >" + data[i].plId + "</td>";
		html += "<td id='goodsName' >" + data[i].goodsName + "</td>";
		html += "<td id='pickNumber' >" + data[i].pickNumber + "</td>";
		html += "<td id='pickingDate' >" + data[i].pickingDate + "</td>";
		html += "<td id='status' >" + data[i].status + "</td>";
		html += "<td ><div><button type='button' PicklistId='"
				+ data[i].plId
				+ "' class='button border-main border-little icon-plus-square-o' data-toggle='modal' data-target='#addPicklist'onclick='update(this)' >修改</button>";
...
function del(obj) {
	...
	if (confirm("您确定要删除吗?")) {
		$.ajax({
			url : "http://localhost:8080/ERP/picklist/delete",
			type : "get",
			data : {
				plId : plId
			},
...
function gotoPage(pageNum, pageSize) {
	$.ajax({
		url : "http://localhost:8080/ERP/picklist/getPageInfo",
		type : "get",
		data : {
			pageNum : pageNum,
			pageSize : pageSize,
...
function pushData() {
	...
	var plMan = $("[name=plMan]").val();
	var plPosition = $("[name=plPosition]").val();
	var pickNumber = $("[name=pickNumber]").val();
	var operator = $("[name=operator]").val();
	var pickingDate = $("[name=pickingDate]").val();
	var remark = $("[name=remark]").val();
	$.ajax({
		url : "http://localhost:8080/ERP/picklist/update",
		type : "post",
		data : {
			plId : plId,
			purpose : purpose,
...
function delSelect() {
	...
	$.ajax({
		url : "http://localhost:8080/ERP/picklist/deleteMany",
		data : {
			id_arr : id_arr
		},
		traditional : true,
...
$(function() {
	...
	$("[name=add]").bind("click", function() {
		...
		$("[name=pickNumber]").val("");
		//$("[name=operator]").val("");
		$("[name=pickingDate]").val("");
		$("[name=remark]").val("");
		$.ajax({
			url : "http://localhost:8080/ERP/picklist/generatePicklistId",
			type : "get",
			success : function(result) {
				if (result.status == 200) {
					$("[name=plId]").val(result.object);
				} else {
					alert(result.message);
				}
			},
	...
	$("#addPicklist").on("hide.bs.modal", function() {
		$(".field").each(function() {
	...
	$("[name=pickingDate]").datetimepicker({
